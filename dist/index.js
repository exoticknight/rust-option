...
function isMatch(thisValue, value, deep) {
    ...
    // Ok(1) matches Ok
    // Err(1) matches Err
    if (thisValue instanceof some)
        return value === Some || (value instanceof some && isMatch(thisValue.unwrap(), value.unwrap(), deep));
    if (thisValue instanceof ok)
        return value === Ok || (value instanceof ok && isMatch(thisValue.unwrap(), value.unwrap(), deep));
    if (thisValue instanceof err)
        return value === Err || (value instanceof err && isMatch(thisValue.unwrapErr(), value.unwrapErr(), deep));
    if (thisValue === exports.None)
